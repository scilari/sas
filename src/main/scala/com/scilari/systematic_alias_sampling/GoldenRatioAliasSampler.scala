...
final class GoldenRatioAliasSampler(pmf: Array[Double], values: Array[Double]) extends SystematicAliasSampler.DoubleSampler(pmf, values, new Parameters()){
  type VALUE_T = Double
  private[this] val generator = new GoldenRatioAliasSampler.GoldenRatioSequenceGenerator()
